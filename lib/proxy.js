...
module.exports = function(cache, options, logger) {
	...
	return function(req, res) {
		...
		if (req.url.substr(0, 2) !== '/~') {
            ...
            logger.info('Proxy request ' + uri.href);

            reqd.add(proxy(uri.href, res, function(r) {
				var id = rsi.identify(r, uri, cache, options, logger),
                    type = r.headers['content-type'];

                logger.info('Got response from ' + uri.href);

                if (!type) {
                    logger.warn('No Content-Type for ' + uri.href);
                }

				if (type && type.match(/^(?:text\/|application\/json)/)) {
